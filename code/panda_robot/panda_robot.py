...
class PandaRobot:
    ...
    def __init__(self, include_gripper):
        ...
        self.robot_id = p.loadURDF("franka_panda/model_description/panda.urdf", useFixedBase=True)
        # panda_model = "panda_with_gripper.urdf" if include_gripper else "panda.urdf"
        # panda_model = "panda.urdf"
    ...
    def calculate_inverse_kinematics(self, position, orientation,lowerLimits,upperLimits,restPoses):
        """"""
        return p.calculateInverseKinematics(self.robot_id, self.dof, position, orientation,lowerLimits = lowerLimits,upperLimits = upperLimits,restPoses = restPoses)
